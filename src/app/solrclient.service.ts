...
export class SolrclientService {
  ...
  baseUrl = environment.SOLRENDPOINT+"/solr/gettingstarted/select";
  
  statsUrl=environment.SOLRENDPOINT+"/solr/gettingstarted/select?q=servicename:Match&stats=true&stats.field={!func}termfreq(%27status%27,%20%27passed%27)&stats.field={!func}termfreq(%27status%27,%27failed%27)&stats.field={!func}termfreq(%27status%27,%27skipped%27)&rows=200&indent=true";
  
  summaryOfTestExecutionResults_facet_pivot_url=environment.SOLRENDPOINT+"/solr/gettingstarted/select?q=servicename:*&facet=true&facet.field=servicename&facet.pivot=servicename,status&indent=true";

  serviceTestStatsByStatus=environment.SOLRENDPOINT+"/solr/gettingstarted/select?q=servicename:{%servicename}&fq=status:{%status}&rows=2000&indent=true";
